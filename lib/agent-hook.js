...
export async function installAgentRouting(ctx, deps) {
  ...
  function attach(agent) {
    ...
    // Start the world decision now, in the background. Correctness does not depend on it —
    // every routed call settles its own path before acting — but the answer crosses SSH, and
    // having it ready before the model's first tool call means the call never waits.
    if (typeof worlds.ensure === 'function') worlds.ensure(cwd).catch(() => {})
