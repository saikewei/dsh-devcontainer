...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * The harness's shared UI primitives, which is where the real code renderer lives.
     *
     * `@deepseek-ai/dsh-client-ui-primitives` is served by the app shell, so any client bundle
     * resolves it by name; the shipped document preview reaches its code viewer through exactly
     * this module. Optional on purpose: a shell without it still gets a readable `<pre>` below
    ...
    const CSS = [
      ...
      '.dshDc_badgeWarn{color:var(--dsw-alias-state-warn-primary)}',
      ...
      // Wrapping is a property of this scrollport. The shared block reads its line white-space
      // from a custom property, so the toggle sets it here and restores the `pre` behaviour its
      // own stylesheet assumes. The doubled class is deliberate: it lifts specificity above the
      // block's `.<hash>_code pre` rule, which is otherwise a coin flip on stylesheet order.
      '.dshDc_fileScroll{flex:1 1 0;min-height:0;overflow:auto;scrollbar-width:thin;white-space:pre}',
      '.dshDc_fileScroll.dshDc_fileWrapped{--dsl-code-block-line-white-space:pre-wrap}',
      '.dshDc_fileScroll.dshDc_fileWrapped pre{white-space:pre-wrap;overflow-wrap:anywhere}',
      '.dshDc_fileScroll pre{white-space:inherit;margin:0}',
      '.dshDc_codeBlock{height:100%;min-height:0}',
    ...
    function PortRow(props) {
      return h('div', { className: 'dshDc_portRow' }, [
        ...
        props.badge === undefined
          ...
          : h('span', {
            key: 'b',
            className: props.badgeWarn === true ? 'dshDc_badge dshDc_badgeWarn' : (props.badgeOn === true ? 'dshDc_badge dshDc_badgeOn' : 'dshDc_badge'),
          }, text(props.badge)),
    ...
    function PortsPanel() {
      ...
      /**
       * Re-bind one live forward, widening or narrowing who can reach it.
       *
       * A listener cannot change the address it accepts on, so this is a stop and a start: the
       * connections it is carrying end, which is why it is an explicit click and not a side
       * effect of anything else. The local port is carried over so the URL does not move.
       */
      const rebind = (entry, bind) => act('bind:' + String(entry.remotePort), async () => {
        await request(API + '/ports', {
          method: 'DELETE',
          headers: { 'content-type': 'application/json', accept: 'application/json' },
          body: JSON.stringify({ port: entry.remotePort }),
        })
        return postJson(API + '/ports', {
      ...
      const body = [
        h('div', { key: 'fwd', className: 'dshDc_section' }, [
          ...
          ...forwards.map((entry) => h(PortRow, {
            ...
            badge: entry.lan === true ? '本网络可见' : (entry.auto === true ? '自动' : undefined),
            badgeOn: true,
            badgeWarn: entry.lan === true,
            actions: [
              h('button', {
                key: 'bind',
                type: 'button',
                className: 'dshDc_btn dshDc_btnSec',
                disabled: busy !== null,
                title: entry.lan === true
                  ? '改为只有本机能访问（会断开现有连接）'
                  : '允许同一网络的其他设备访问，例如手机（会断开现有连接）',
                onClick: () => void rebind(entry, entry.lan === true ? '127.0.0.1' : '0.0.0.0'),
              }, entry.lan === true ? '收回' : '共享'),
              h('button', {
                key: 'open',
                type: 'button',
                className: 'dshDc_btn dshDc_btnSec',
    ...
    /**
     * Extension → grammar hint, copied from the shipped code previewer's own table.
     *
     * The highlighter wants a GRAMMAR name, not a suffix: handing it "ts" instead of
     * "typescript" is how a file renders without highlighting. This is a copy rather than an
     * import because the table lives inside the preview package's bundle and is not exported.
     *
    ...
    /** Suffix → grammar hint, flattened once. */
    const GRAMMAR_BY_EXTENSION = new Map(
      Object.entries(CODE_LANGUAGES).flatMap(([language, extensions]) =>
        extensions.map((extension) => [extension, language])),
    )

    /**
     * The highlighter grammar for a path, or undefined when the suffix names none.
     *
     * Only the LAST suffix counts, and a dotfile has none: `.gitignore` matches no grammar and
     * renders as plain numbered text, which is the shipped behaviour too.
     */
    function languageForPath(path) {
      const found = /\.([^./]+)$/u.exec(text(path).replaceAll('\\', '/'))
      const extension = found === null ? undefined : found[1].toLowerCase()
      return extension === undefined ? undefined : GRAMMAR_BY_EXTENSION.get(extension)
    }
     ...
     *
     * Only the FETCH is ours. The drawing is the harness's own code renderer — the very
     * `CodeBlock` the shipped document preview draws its code files with — reached through the
     * shared primitives module rather than through the preview's slot. Going through the slot
     * was tried first and does not work here: `sidebar.right.tab.document` dispatches to a
     * REGISTERED BODY by id, and its default body for text renders no visible line numbers at
     * all (its per-line `data-` attribute is a scroll target, not a number). The code body is
     * the one that passes `lineNumbers: true` and a grammar, so this asks for that component
     * directly and inherits both.
     *
     * Wrapping is this component's own: the block reads a `--dsl-code-block-line-white-space`
     * custom property, so the toggle only has to set it on this scrollport and restore the
     * `pre` behaviour its stylesheet assumes.
     */
    function ContainerFileBody(props) {
      ...
      // The drawing is the harness's own code renderer, reached through the module the shipped
      // previewer itself uses — so line numbers, syntax highlighting and the copy control are
      // the original implementation's rather than a lookalike. The `<pre>` below is only for a
      // shell that does not serve that module: a file stays readable there, without line numbers.
      const codeBlock = primitives === undefined ? undefined : primitives.CodeBlock
      const body = payload.text === ''
        ? h('div', { key: 'm', className: 'dshDc_fileMsg' }, '这个文件是空的。')
        : typeof codeBlock !== 'function'
          ? h('pre', { key: 'plain', className: 'dshDc_fileCode' }, text(payload.text))
          : h(codeBlock, {
            key: 'code',
            className: 'dshDc_codeBlock',
            // `contentRef`, not `ref`: the block scrolls an inner element and hands that out.
            contentRef: scrollport,
            code: text(payload.text),
            lang: languageForPath(path),
            lineNumbers: true,
            copyLabel: '复制',
            copiedLabel: '已复制',
            // Everything we serve is a complete read of what the route returned, truncated or
            // not, so the block is never mid-stream.
            streaming: false,
          })
    ...
    async function apply(ctx) {
      ...
      ctx.inject(['slots', 'sidebarRightTabs'], (scope) => {
        ...
        // above and not the kind. Nothing else is injected: the body draws with the shared
        // primitives module it required at load time, so it needs no service from the preview.
