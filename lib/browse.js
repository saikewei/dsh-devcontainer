      ...
      if (route === '/ports') {
        ...
        if (req.method === 'POST') {
          ...
          const started = await forwards.add({
            ...
            // The panel's "this network" switch. Validated where every other bind is, so a bad
            // address answers 400 here instead of reaching `server.listen`.
            bind: body.bind === undefined ? undefined : String(body.bind),
          })
          if (started.ok !== true) {
