...
* **画的是原版查看器，不是仿制品。** 我们只负责取字节：tab 把文本交给 harness 本来就带的 document body，走公开的 `sidebar.right.tab.document` 槽——它的 `ownerProps` 记录为 `DocumentContent`，是该槽公开契约的一部分。所以行号、逐行 DOM、定位行高亮都是**原实现的**；而且某个部署若为某扩展名注册了自己的查看器，这里也会用上它——实现是按预览器自己的排序规则选的，不是写死的 id。
* 折行是本 tab 自己滚动容器上的一条 `white-space`（原版的 page 元素特意设成 `white-space: inherit`，就是为了让宿主自己决定），所以折行开关和刷新一起放在本 tab 的头部，默认开启，与原版一致。
* 只处理文本，且只取前 2 MB。更长的文件**截断**提供而不是拒绝——对一份日志来说，开头才是有用的答案；二进制文件则以该理由拒绝，而不是渲染成乱码。

空文件、失败状态，以及没挂载原版预览器的部署，回退到本插件自己的 `<pre>`——文件始终可读，只是那里没有行号。
