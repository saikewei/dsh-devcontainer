...
export async function apply(ctx, config) {
  ...
  if (wantsRouting && !hasStandIn) {
      ...
      const refreshRoots = () => {
        ...
        try {
          worlds.setKnownRoots(
            registry
              .list()
              .map((workspace) => worlds.toContainer(workspace.path))
              .filter((containerPath) => containerPath !== undefined),
          )
        } catch {
          // A registry that is not ready yet is not an error; the next tick retries.
        }
      }
      ctx.inject(['workspaceRegistry'], (scoped) => {
        refreshRoots()
        const tick = setInterval(refreshRoots, 5000)
    ...
    ctx.effect(() => registerContainerApi(ctx, channel, worlds, cfg))
  }
