...
export async function apply(ctx, config) {
  ...
  if (cfg.provideFs || cfg.provideShell) {
    if (cfg.mountPoint === '' && cfg.mountRoot === '') {
      console.error(
      ...
      // with node:fs realpath, which never sees the container. Creating them here means
      // the row is self-sufficient on a fresh machine.
      ...
      // The picker is useful even where routing is off, so it is registered here rather
      // than gated on the web profile: it reports its own `routing` fact to the client.
