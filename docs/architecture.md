...
## 附九：把转发按需暴露给局域网（2026-09）

需求是"代理一个服务，让手机也能访问"。原设计只绑 `127.0.0.1`，而且 `forwardBind` 是**全局**的——
打开就等于把所有转发一起发布到网络。

### 数据路径（先澄清一件事）

```
手机 → Mac:<localPort> → 插件的本地监听 → 常驻通道 → 容器内 helper → connect 127.0.0.1:<容器端口>
```

监听在 **DSH 所在的那台机器**上，不是 NAS、也不是容器。所以手机必须和**运行 DSH 的那台机器**
在同一局域网。NAS 侧没有任何监听：容器里的服务绑的是容器自己的 loopback。

### 做法：bind 从"全局配置"变成"每个转发的属性"

`Forwards.add()` 接受 `bind`，默认落在 profile 的 `forwardBind` 上；`#listen`、`#label`、`#describe`
一律读 `entry.bind`。工具侧 `devc_forward` 多了 `bind` 参数，面板侧 `POST /ports` 多了同一个字段，
转发行上多了一个**共享 / 收回**按钮（换绑地址无法在活着的监听上迁移，所以那个动作是"停掉再建"，
并在 tooltip 里说明会断开现有连接）。

**URL 必须跟着修**：`http://0.0.0.0:8000` 什么也打不开。`reachableHostFor()` 于是分两种——
具体地址就是"它监听的地方"，本身即答案；通配地址则取本机第一个非 internal 的 IPv4 顶替
（没有就退回 loopback，至少本地为真）。IPv6 字面量在 URL 里加方括号。

**校验**放进 `normalizeBind()`：IP 字面量或 `localhost`，其余拒绝。bind 会直接进
`server.listen(port, address)`，危险的形态倒不是问题（不是 argv），**一个拼错却静默绑到别处**的
地址才是。

### 有意的取舍

**已有转发不会被重新绑定。** 同一个 key 再次 `add` 返回现有条目，而不是换地址——换地址意味着丢掉
它正在承载的每一条连接，这必须由调用者显式地先停掉。面板上那个按钮做的正是这件事。

**没有做鉴权。** 转发是一条裸 TCP relay，`#attach` 不检查来源地址。所以"共享"的真实含义是
"把容器里那个服务原样给整个网络，且它自己没有认证"。这一点在工具输出里明说，面板上用
`--dsw-alias-state-warn-primary` 的徽章标出来，而不是让人从 bind 地址去推断。

### 验收

`test/ports.mjs` 新增 19 条断言：默认值不被污染、通配与具体地址各自的 URL 规则、IPv6 方括号、
非地址输入被拒且**没有留下任何监听**、以及"本机不持有的地址要明确失败而不是绑到别处"
（用保留网段 TEST-NET-1 验证）。7 条变异全部咬住。

测试里**故意不绑局域网地址**——那会在测试机上真的开一个对外监听。`0.0.0.0` 那条已经覆盖了
`lan` 标记的同一个分支。
