...
export async function apply(ctx, config) {
  ...
  // `provideSearch` and `prompt` are only read inside the routing branch below, so setting
  // one without routing registers nothing at all. Say so rather than silently ignoring it.
  const gatedButOff = [
    cfg.provideSearch ? 'provideSearch' : '',
    cfg.prompt ? 'prompt' : '',
  ].filter(Boolean)
  if (!wantsRouting && gatedButOff.length > 0) {
    console.error(
      'dsh-devcontainer: ' + gatedButOff.map((k) => '`' + k + '`').join(' and ')
      + ' ha' + (gatedButOff.length > 1 ? 've' : 's') + ' no effect without routing — '
      + (gatedButOff.length > 1 ? 'they are' : 'it is') + ' read only when `provideFs` or '
      + '`provideShell` is on.',
    )
  }
