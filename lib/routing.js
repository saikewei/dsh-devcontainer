...
/**
 * The two-world path mapping: a local mount-point subtree that stands for the
 * container's own root. Both spellings resolve to the same container path, so a
 * target keeps one identity however the caller spelled it.
 */
export class Worlds {
  #mountPoint
  #containerRoot
  ...
  /** Map an absolute path in either spelling to its container path, or undefined for a local path. */
  toContainer(absolute) {
    if (absolute === undefined) return undefined
    const path = stripTrailing(normalize(absolute))
    if (path === this.#containerRoot || isUnder(this.#containerRoot, path)) return path
    if (this.#mountPoint !== undefined && (path === this.#mountPoint || isUnder(this.#mountPoint, path))) {
  ...
  /** Express a container path in the mount-point spelling, when one is configured. */
  toMount(containerPath) {
    if (this.#mountPoint === undefined) return containerPath
    if (containerPath === this.#containerRoot) return this.#mountPoint
    if (isUnder(this.#containerRoot, containerPath)) {
