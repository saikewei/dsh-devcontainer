...
/**
 * The machine a tool call names, checked against the roster this profile offers.
 *
 * The roster is `~/.ssh/config` plus `extraHosts` plus the configured `sshHost`, which is the
 * same list the workspace picker offers and the same one its HTTP route enforces. A tool
 * argument reaches `ssh` as a DESTINATION, so without this a name the operator never offered
 * would still be dialled — through whatever `Host *` block happens to match it.
 *
 * A roster that could not be read is not treated as an empty roster: the configured host and
 * `extraHosts` still answer, everything else is refused with the reason attached.
 *
 * @param cfg - the resolved plugin config.
 * @param requested - the tool argument, or undefined for the configured default.
 * @returns the destination to dial.
 * @throws when the name is not one this profile offers.
 */
export async function pickRosterHost(cfg, requested) {
  const wanted = requested === undefined || String(requested).trim() === ''
    ? cfg.sshHost
    : String(requested).trim()
  const roster = await listHosts(cfg)
  if (roster.hosts.some((entry) => entry.alias === wanted)) return wanted
  throw new Error(
    'unknown host "' + wanted + '": it is not a machine this profile offers'
    + (roster.error === undefined ? '' : ' (the ssh config could not be read: ' + roster.error + ')'),
  )
}
...
function registerHostTools(ctx, forTarget, containersFor, cfg) {
  ...
  /**
   * The host channel a call names, checked against the roster first.
   *
   * Asynchronous because the check reads `~/.ssh/config`; every caller is an `execute`, so the
   * await costs nothing but keeps one gate for all five tools instead of five copies.
   */
  const channelFor = async (requested) => {
    const host = await pickRosterHost(cfg, requested)
    return forTarget({ host, world: 'host' })
  }
  const report = (channel) => (error) =>
    '[dsh-devcontainer error] ' + String(error && error.message ? error.message : error)
    + (channel === undefined || channel.lastError === undefined
      ? ''
      : NL + '[host channel ' + channel.config.sshHost + '] ' + channel.lastError)
  ...
  ctx.tools.register({
    ...
    async execute(args, exec) {
      let channel
      try {
        channel = await channelFor(args.host)
  ...
  ctx.tools.register({
    ...
    async execute(args, exec) {
      ...
      try {
        channel = await channelFor(args.host)
        const frame = await channel.request({ op: 'read', path: String(args.path) }, 60000, exec && exec.signal)
        const all = String(frame.text).split(NL)
        const offset = args.offset === undefined ? 1 : Math.max(1, Number(args.offset))
        const limit = args.limit === undefined ? 2000 : Math.max(1, Number(args.limit))
        const window = all.slice(offset - 1, offset - 1 + limit)
  ...
  ctx.tools.register({
    ...
    async execute(args, exec) {
      ...
      try {
        channel = await channelFor(args.host)
        const frame = await channel.request({ op: 'list', path: String(args.path) }, 60000, exec && exec.signal)
        const entries = frame.entries ?? []
        if (entries.length === 0) return '[empty directory]'
        return entries
          .map((entry) => {
  ...
  ctx.tools.register({
    ...
    async execute(args) {
      try {
        const target = await pickRosterHost(cfg, args.host)
...
function registerPortTools(ctx, forwards, cfg) {
  ...
  ctx.tools.register({
    ...
    output: OUTPUT_TEXT,
    async execute() {
      const lines = ['forwards (default bind ' + cfg.forwardBind + '):']
      ...
      for (const forward of active) {
        lines.push('  ' + forward.localAddress + ' -> ' + forward.container + ':' + String(forward.remotePort)
          + (forward.auto ? '  [auto]' : '')
          // A forward anybody on the network can reach is not a detail to leave to the bind
          // column: it changes who can use the service.
          + (forward.lan === true ? '  [on this network]' : '')
  ...
  ctx.tools.register({
    ...
    description:
      ...
      + 'to the container\'s own loopback works. Returns the local URL to open. By default only '
      + 'this machine can reach it; pass `bind` to widen that.',
    parameters: parameters({
      ...
      bind: {
        type: 'string',
        description: 'Local address to accept on. Defaults to the configured `forwardBind` '
          + '(loopback). `0.0.0.0` accepts from every device that can route to this machine — '
          + 'the container\'s service is then on your network with no authentication of its own.',
      },
      host: { type: 'string', description: 'SSH destination, when not the configured one.' },
    }),
    output: OUTPUT_TEXT,
    async execute(args) {
      // Same gate as the host tools: the destination is a tool argument, so it is checked
      // against the roster rather than dialled as given.
      const target = await pickRosterHost(cfg, args.host)
      const started = await forwards.add({
        ...
        bind: args.bind === undefined ? undefined : String(args.bind),
      })
      ...
      return 'forwarding ' + forward.url + '  ->  ' + forward.container + ':' + String(forward.remotePort)
        + (forward.lan === true
          ? NL + 'reachable from other devices on this network (listening on ' + forward.localAddress
            + '), and it carries no authentication of its own'
          : '')
        + (forward.substituted
...
export async function apply(ctx, config) {
  ...
  // One SSH transport per machine actually in use, created on first reference: a deployment
  // may list a dozen ssh hosts and touch one, and both execution worlds — the container and the
  // host that runs it — are reached over the same one.
  const transports = new Map()
  const transportFor = (host) => {
    const name = host === undefined || host === '' ? cfg.sshHost : host
    let found = transports.get(name)
    if (found === undefined) {
      found = new RemoteTransport(name)
