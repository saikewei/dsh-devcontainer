...
一轮结束时那条"本轮文件改动"记录的是模型传给 `write`/`edit` 的 `file_path`。在路由会话里那是一个**容器**路径——而所有随包提供的打开入口都要经 `workspaceFiles` → `ctx.fs` → 本插件的空本地替身，所以那些 chip 解析不到任何东西。为它们再注册一个侧边栏 tab 类型，就是给它们一个落点：

* 它以 **`extension`** 档声明 `dsh-resource://file/session/**`，优先级高于随包文本预览器的 `fallback`，并且否决一切不落在本 profile 已路由根之下的地址。所以本地文件仍然由随包预览器打开，行为不变。
* 该 tab 经 `GET /dsh-devcontainer/file` 读取，这条路由用的是工具所用的同一个 `worlds.locate` 来解析机器与容器。浏览器不指定机器、也不指定容器，所以它送来的东西无需被信任。
* **代码由 harness 自己的渲染器画。** 我们只负责取字节：tab 渲染的是 `@deepseek-ai/dsh-client-ui-primitives` 里的 `CodeBlock`——正是随包预览器画代码文件用的那个组件——所以行号、语法高亮、复制按钮和滚动行为都是**原实现的**，不是仿制品。语法由文件后缀决定，用的是与预览器同一张扩展名→语言表。
* 折行是本 tab 自己滚动容器上的一条 `white-space`，通过该 block 的 `--dsl-code-block-line-white-space` 自定义属性驱动。折行开关和刷新一起放在本 tab 的头部，默认开启，与原版一致。
* 只处理文本，且只取前 2 MB。更长的文件**截断**提供而不是拒绝——对一份日志来说，开头才是有用的答案；二进制文件则以该理由拒绝，而不是渲染成乱码。

空文件与失败状态就地显示原因。若某个 shell 不提供 primitives 模块，则回退到本插件自己的 `<pre>`——文件仍可读，只是没有行号。

收尾正文里以行内代码写出的文件引用走同一个 `openFile`，因此一并可用。
...
* **路由会话不止能到容器，也能到宿主。** 三条路：那四个 `devc_host_*` 工具；镜像下任何"所在目录没有 dev container"的路径，被影子化的 `bash`/`read`/`write` 会直接在宿主上跑；以及传输层本身就是 `ssh <host>`，用的是你自己的 ssh 配置——所以权限就是那个 ssh 用户的权限。宿主命令**不受沙箱约束**：路由执行器的远端分支直接走通道，从不经过本地那个带沙箱的执行器，后者只看得到本地路径。
* 带 host 参数的工具会先过本 profile 提供的 roster（`~/.ssh/config` 加 `extraHosts` 加配置里的 `sshHost`），也就是工作区选择器展示的同一份名单。以 `-` 开头的目标一律直接拒绝：ssh 会把这种参数当**选项**解析，而 `-oProxyCommand=…` 会在**本机**执行命令。
* 容器侧写入按设计绕过本地沙箱。本插件不改变任何本地路径行为，而且路由会话的本地分支就是随包实现本身。
* 只有那七个工具会被路由。侧边栏文件树、技能发现等直接读 `ctx.fs` 的消费者，对镜像工作区看到的仍是本地替身——这是"永不替换全局服务"的代价。要读容器内部，用 `devc_read`/`devc_ls`/`devc_glob`/`devc_grep`。
  唯一的例外是**打开被改动的文件**，因为它不走 `ctx.fs`：上面那个 tab 经本插件自己的路由从容器取得内容。
* `present` 声明与 `@` 文件选择器都经本地 `ctx.fs` 解析，因此两者都无法指向容器文件。
* 路由读写不经过 `ctx.fs`，所以"先读后写"的观察策略看不到它们。它不是容器路径的守卫。
* 一条被转发的连接，字节是以 base64 装在 JSON 行里走常驻通道的——比净荷多约三分之一流量，且和文件工具共用同一条管道。对开发服务器这是划算的取舍，搬大文件则不是。

## 开发

十六套测试里有十一套无容器，任何地方都能跑：

```sh
npm install
npm run test:unit       # 不需要目标
npm test                # 全部十六套；其余需要真实 dev container
```
