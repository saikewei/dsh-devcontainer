...
export async function apply(ctx, config) {
  ...
  if (unconfigured.length > 0) {
    // Name what actually breaks: without `sshHost` nothing can reach the machine at all,
    // while a missing `container` still leaves the host-level tools usable.
    const breaks = cfg.sshHost === ''
      ? 'No tool can reach the machine until you do.'
      : 'The container tools (devc_exec, devc_read, …) will fail until you do; the devc_host_* tools still work.'
    console.error(
      'dsh-devcontainer: ' + unconfigured.map((key) => '`' + key + '`').join(' and ')
      + ' not configured — set ' + (unconfigured.length > 1 ? 'them' : 'it')
      + ' on the `devcontainer` row of your profile. ' + breaks,
    )
  }
