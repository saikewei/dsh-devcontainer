...
export class Forwards {
  ...
  #attach(entry, socket) {
    ...
    entry.relays.set(relayId, { socket, off })

    socket.on('data', (chunk) => {
      if (channel.notify({ event: 'relay_data', relayId, b64: chunk.toString('base64') })) return
      // The pipe is full. Pausing this socket is real backpressure all the way to the browser.
      socket.pause()
    ...
    channel.request(
      ...
      // Logged, because the browser only sees a connection that closed, and that says nothing
