 ...
 * still a perfectly good place to keep a project — `nas` has Node, `eu` does not.
 */
...
export function registerContainerApi(ctx, deps) {
  const { forTarget, transportFor, containersFor, worlds, cfg } = deps
  ...
  const pickHost = (requested) =>
    requested === undefined || String(requested) === '' ? cfg.sshHost : String(requested)
  ...
  const handler = async (req, res) => {
    try {
      ...
      if (req.method === 'GET' && route === '/list') {
        const host = pickHost(url.searchParams.get('host'))
