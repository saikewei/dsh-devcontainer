...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function ContainerDirectoryFlow(props) {
      ...
      const [world, setWorld] = React.useState('remote')
      ...
      const browse = React.useCallback(async (target, machine) => {
        setLoading(true)
        setError(null)
        try {
      ...
      React.useEffect(() => {
        ...
        getJson(API + '/config').then((loaded) => {
          if (!alive.current) return
          setConfig(loaded)
          setHost(loaded.defaultHost || '')
          return browse(loaded.browseRoot || '/', loaded.defaultHost || '')
        }, (reason) => {
          if (alive.current) setError(reason instanceof Error ? reason.message : String(reason))
        })
      }, [open, browse])
      ...
      const body = []
      if (config !== null && config.hosts.length > 0) {
        const options = config.hosts.map((entry) =>
          h('option', { key: entry.alias, value: entry.alias },
            entry.alias + (entry.hostName === undefined ? '' : '  ·  ' + (entry.user ? entry.user + '@' : '') + entry.hostName)))
        body.push(h('div', { key: 'host', style: styles.pathRow },
          h('span', { style: styles.muted, title: config.sshConfigPath }, '主机'),
          h('select', {
            style: styles.select,
            value: host,
            disabled,
            onChange: (event) => {
              ...
              setHost(next)
              setListing(null)
              browse(config.browseRoot || '/', next)
            },
          }, options),
          config.hostsError === undefined
            ? null
            : h('span', { style: styles.error, title: config.hostsError }, config.hostsError.slice(0, 24)),
        ))
      }
      ...
      body.push(h('div', { key: 'path', style: styles.pathRow },
        h('button', {
          ...
          onClick: () => { if (listing && listing.parent !== undefined) browse(listing.parent, host) },
        }, '上一级'),
      ...
      if (config !== null) {
        body.push(h('div', { key: 'where', style: styles.note },
          host + ' 上的宿主目录；带标记的目录含有 .devcontainer',
        ))
      }
      ...
      if (loading) rows.push(h('div', { key: 'loading', style: styles.muted }, '读取中…'))
        ...
        for (const entry of entries) {
          rows.push(h('button', {
            ...
            style: styles.row,
            disabled,
            onClick: () => browse(entry.path, host),
