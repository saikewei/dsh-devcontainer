 ...
 */
function registerHostTools(ctx, hostChannel, containers, cfg) {
  const hostRoot = cfg.hostRoot || '/'
  const report = (error) =>
    '[dsh-devcontainer error] ' + String(error && error.message ? error.message : error)
    + (hostChannel.lastError ? NL + '[host channel] ' + hostChannel.lastError : '')
  ...
  ctx.tools.register({
    ...
    async execute(args, exec) {
      try {
        ...
        const frame = await hostChannel.request({
          op: 'exec',
  ...
  ctx.tools.register({
    ...
    async execute(args, exec) {
      try {
        const frame = await hostChannel.request({ op: 'read', path: String(args.path) }, 60000, exec && exec.signal)
        const all = String(frame.text).split(NL)
        const offset = args.offset === undefined ? 1 : Math.max(1, Number(args.offset))
        const limit = args.limit === undefined ? 2000 : Math.max(1, Number(args.limit))
        const window = all.slice(offset - 1, offset - 1 + limit)
  ...
  ctx.tools.register({
    ...
    async execute(args, exec) {
      try {
        const frame = await hostChannel.request({ op: 'list', path: String(args.path) }, 60000, exec && exec.signal)
        const entries = frame.entries ?? []
        if (entries.length === 0) return '[empty directory]'
        return entries
          .map((entry) => {
    ...
    async execute(args) {
      try {
        if (args.folder === undefined) {
          ...
          if (found.length === 0) return '[no dev containers on ' + cfg.sshHost + ']'
          return found
            .map((entry) =>
...
export async function apply(ctx, config) {
  ...
  const transport = new RemoteTransport(ctx, cfg)
  const channel = new RemoteChannel(transport, cfg, 'container')
  const hostChannel = new RemoteChannel(transport, cfg, 'host')
  const containers = new DevContainers(transport)

  if (cfg.tools) registerTools(ctx, channel, cfg)
  if (cfg.hostTools) registerHostTools(ctx, hostChannel, containers, cfg)
  ...
  if (wantsRouting && !hasStandIn) {
    ...
    if (wantsRouting) {
      ...
      // Which channel reaches which world.
      const channels = {
        forWorld: (world) =>
          world === 'host' ? hostChannel : world === 'container' ? channel : undefined,
      }
      ...
      const refreshWorlds = async () => {
        ...
        try {
          ...
          for (const workspace of registry.list()) {
            const hostFolder = worlds.mirrorPath(workspace.path)
            if (hostFolder === undefined) {
              // The explicit container pair, or an ordinary local workspace: nothing to look up.
              const containerPath = worlds.toContainer(workspace.path)
              if (containerPath !== undefined) {
                entries.push({ localPrefix: workspace.path, world: 'container', remotePath: containerPath })
              }
              continue
            }
            let resolved = containerCache.get(hostFolder)
            if (resolved === undefined || resolved.usable !== true) {
              try {
                resolved = await containers.resolve(hostFolder)
                containerCache.set(hostFolder, resolved)
              } catch {
                resolved = undefined
              }
            }
            if (resolved !== undefined && resolved.usable === true) {
              ...
              entries.push({ localPrefix: workspace.path, world: 'host', remotePath: hostFolder })
            }
          }
          worlds.setResolved(entries)
          worlds.setKnownRoots(
            entries.filter((entry) => entry.world === 'container').map((entry) => entry.remotePath),
          )
        } catch {
      ...
      if (cfg.provideFs) ctx.plugin(createRoutingFileSystem(channels, worlds))
      if (cfg.provideShell) ctx.plugin(createRoutingShellExecutor(channels, worlds))
    ...
    ctx.effect(() => registerContainerApi(ctx, hostChannel, transport, containers, worlds, cfg))
  }
