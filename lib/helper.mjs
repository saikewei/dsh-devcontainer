...
import { readFile, writeFile, stat, lstat, readdir, realpath, rename, mkdir } from 'node:fs/promises'
...
function run(command, cwd, timeoutMs, stdinText) {
  return new Promise((resolve) => {
    const child = spawn('/bin/bash', ['-c', command], { cwd: cwd || undefined })
    const outSink = { value: '' }
    const errSink = { value: '' }
    let spilled = false
  ...
  try {
    switch (request.op) {
      ...
      case 'write': {
        ...
        const temp = request.path + '.dsh-tmp-' + randomBytes(4).toString('hex')
        await writeFile(temp, request.text, 'utf8')
        await rename(temp, request.path)
      ...
      case 'grep': {
        ...
        const command = 'grep -rn -E' + include + ' -- ' + shq(request.pattern) + ' ' + shq(request.path) + ' | head -250'
        const res = await run(command, request.cwd, 60000)
        result = { ok: true, text: res.stdout, cap: 250 }
        break
      }
      case 'glob': {
        const command = "shopt -s globstar nullglob dotglob; for f in " + request.pattern + '; do [ -f "$f" ] && echo "$f"; done | head -200'
        const res = await run(command, request.cwd || '/', 60000)
        result = { ok: true, text: res.stdout, cap: 200 }
        break
      }
      default:
