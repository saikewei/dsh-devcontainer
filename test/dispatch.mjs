...
const container = recorder('container')
const channels = {
  forWorld: (world) => (world === 'host' ? host : world === 'container' ? container : undefined),
}

const worlds = new Worlds({ containerRoot: CONTAINER_ROOT, mountRoot: MOUNT_ROOT })
worlds.setResolved([
  // The mirror folder that has a dev container…
  { localPrefix: MOUNT_ROOT + HOST_FOLDER, world: 'container', remotePath: CONTAINER_PATH },
  // …and one that does not.
  { localPrefix: MOUNT_ROOT + '/volume1/docker/redis', world: 'host', remotePath: '/volume1/docker/redis' },
])
...
const inside = await fs.resolve(MOUNT_ROOT + HOST_FOLDER + '/cmd/main.go')
check('resolves into the container world', worldOf(inside) === 'container', worldOf(inside))
...
check('readText content came from the container channel', text.startsWith('container says'), text)

console.log('\n-- a mirror folder with no dev container --')
host.calls.length = 0
container.calls.length = 0
const onHost = await fs.resolve(MOUNT_ROOT + '/volume1/docker/redis/src/app.go')
check('resolves into the host world', worldOf(onHost) === 'host', worldOf(onHost))
...
host.calls.length = 0
container.calls.length = 0
const entries = await fs.listDir(await fs.resolve(MOUNT_ROOT + HOST_FOLDER))
check('the listing came from the container channel', container.last()?.op === 'list')
check('a child target stays in the container world', entries.length === 1 && worldOf(entries[0].target) === 'container', entries[0] === undefined ? '(no entries)' : worldOf(entries[0].target))
check(
...
host.calls.length = 0
container.calls.length = 0
const containerSpec = shell.resolve({ command: 'echo hi', workdir: MOUNT_ROOT + HOST_FOLDER })
...
const hostSpec = shell.resolve({ command: 'echo hi', workdir: MOUNT_ROOT + '/volume1/docker/redis' })
