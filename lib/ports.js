...
/**
 * Accept a bind address from a caller, or say why not.
 *
 * The value reaches `server.listen(port, address)`, which resolves a hostname and fails with
 * EADDRNOTAVAIL for an address this machine does not hold — so the dangerous shapes are not the
 * worry; a typo that silently binds something unexpected is. An IP literal or `localhost` is
 * the whole vocabulary a bind address has.
 */
function normalizeBind(raw, fallback) {
  if (raw === undefined || raw === null || String(raw).trim() === '') return { bind: fallback }
  const bind = String(raw).trim()
  if (bind === 'localhost' || isIP(bind) !== 0) return { bind }
  return { error: 'not a bind address: ' + JSON.stringify(raw) + ' (an IP literal or localhost)' }
}

/**
 * The host a caller elsewhere on the network should use, for a forward bound to `bind`.
 *
 * A concrete address is exactly where it listens, so it is its own answer. A WILDCARD is not:
 * `http://0.0.0.0:8000` opens nothing, so the machine's first non-internal IPv4 stands in. When
 * even that is missing the loopback answer is returned, which is at least true locally.
 */
function reachableHostFor(bind) {
  if (!isWildcard(bind)) return bind
  for (const addresses of Object.values(networkInterfaces())) {
    for (const address of addresses ?? []) {
      if (address.family === 'IPv4' && address.internal !== true) return address.address
    }
  }
  return '127.0.0.1'
}

/** A URL host, bracketed when it is an IPv6 literal. */
...
export class Forwards {
   ...
   */
  async add({ host, container, port, localPort, bind, auto = false }) {
    ...
    const resolved = normalizeBind(bind, this.#bind)
    if (resolved.error !== undefined) return { ok: false, error: resolved.error }
    const key = this.#key(host, container, remotePort)
    const existing = this.#entries.get(key)
    // A forward that already exists is returned as-is rather than re-bound: swapping a live
    // listener's address means dropping every connection it carries, which is a decision the
    // caller has to make explicitly by stopping it first.
    ...
    const listening = await this.#listen(server, wanted, resolved.bind)
    if (listening.error !== undefined) {
      server.close()
      return { ok: false, error: 'could not listen on ' + resolved.bind + ':' + String(wanted) + ' — ' + listening.error }
    }
  ...
  async #listen(server, startPort, bind, attempts = 20) {
  ...
  #label(entry) {
    return entry.bind + ':' + String(entry.localPort) + ' -> ' + entry.host + ':' + entry.container + ':' + String(entry.remotePort)
  }

  #describe(entry) {
    const host = reachableHostFor(entry.bind)
    // `lan` is not decoration: a forward on anything but loopback is reachable by every device
    // that can route here, and the panel and the tool both say so because of this flag.
    const lan = !isLoopback(entry.bind)
    return {
      host: entry.host,
      container: entry.container,
      ...
      localAddress: entry.bind + ':' + String(entry.localPort),
      url: 'http://' + urlHost(host) + ':' + String(entry.localPort),
    }
  }
