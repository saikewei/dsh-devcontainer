...
/**
 * A target key carries its world, because the world is not recoverable from the path alone:
 * `/etc/hosts` exists on the host AND inside the container. The separator is a NUL, which
 * cannot occur in a path.
 */
const KEY_SEP = String.fromCharCode(0)
const makeKey = (world, path) => world + KEY_SEP + path

function parseKey(key) {
  const text = String(key)
  const index = text.indexOf(KEY_SEP)
  if (index === -1) return undefined
  return { world: text.slice(0, index), path: text.slice(index + 1) }
}
...
/** A target's remote world, its path there, and the channel that reaches it. */
function remoteOf(channels, target) {
  const parsed = parseKey(target.targetKey)
  if (parsed === undefined) return undefined
  const channel = channels.forWorld(parsed.world)
  return channel === undefined ? undefined : { world: parsed.world, path: parsed.path, channel }
}
...
export class Worlds {
  ...
  #resolved = []
  /** Raw host spellings already handed to the model, so they resolve back to the host. */
  #hostRoots = []
   ...
   *
   * @param entries - `{ localPrefix, world: 'host'|'container', remotePath }`.
   */
  setResolved(entries) {
    ...
    this.#hostRoots = this.#usableRoots(
      this.#resolved.filter((entry) => entry.world === 'host').map((entry) => entry.remotePath),
    )
  }
  ...
  toMountRootPath(remotePath) {
    if (this.#mountRoot === undefined) return undefined
    return stripTrailing(this.#mountRoot + remotePath)
  }

  /**
   * The host path a local stand-in mirrors, derived structurally — independent of any
   * registry entry or already-resolved world, which is what makes it usable WHILE resolving.
   *
   * @returns the host path, or undefined when the path is not under the mirror.
   */
  mirrorPath(absolute) {
    if (this.#mountRoot === undefined) return undefined
  ...
  locate(absolute) {
    ...
    for (const entry of this.#resolved) {
      ...
      return { world: entry.world, path: rest === '' ? entry.remotePath : stripTrailing(entry.remotePath + rest) }
    }

    if (this.#mountRoot !== undefined && isUnder(this.#mountRoot, path)) {
      const rest = path.slice(this.#mountRoot.length)
      return { world: 'host', path: rest === '' ? '/' : rest }
    }

    if (this.#mountPoint !== undefined && isUnder(this.#mountPoint, path)) {
      const rest = path.slice(this.#mountPoint.length)
      return { world: 'container', path: stripTrailing(this.#containerRoot + rest) }
    }

    for (const root of this.#roots) if (isUnder(root, path)) return { world: 'container', path }
    for (const root of this.#hostRoots) if (isUnder(root, path)) return { world: 'host', path }
    return undefined
  }
 ...
 * @param channels - `{ forWorld(world) }`, resolving a target's channel.
...
export function createRoutingFileSystem(channels, worlds) {
  return class RoutingFileSystem extends SandboxedFileSystem {
    async resolve(path, opts) {
      ...
      return { targetKey: FsTargetKey(makeKey(located.world, located.path)), displayPath: located.path }
    }
    ...
    contains(parent, child) {
      ...
      if (a === undefined || b === undefined || a.world !== b.world) return super.contains(parent, child)
    ...
    async lstat(path, opts, signal) {
      ...
      const remote = remoteOf(channels, { targetKey: makeKey(located.world, located.path) })
    ...
    async listDir(target, signal) {
      ...
      return (frame.entries ?? []).map((entry) => ({
        ...
        target: {
          ...
          targetKey: FsTargetKey(makeKey(remote.world, base + '/' + entry.name)),
  ...
  return class RoutingBashExecutor extends SandboxBashExecutor {
    resolve(request) {
      ...
      if (located !== undefined) {
        spec[REMOTE_WORLD] = located.world
    ...
    async run(spec) {
      const channel = channels.forWorld(spec[REMOTE_WORLD])
